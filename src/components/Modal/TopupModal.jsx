...
const TopupModal = () => {
  ...
  const [pickedDenom, setPickedDenom] = useState(null);
  ...
  const openTopupModal = () => {
    Swal.fire({
      ...
      html: `
                    <div class="w-full bg-white rounded-lg rounded-br-none z-50 h-full flex flex-col gap-5">
                              ...
                              (i, index) =>
                                `
                                    <button 
                                      ...
                                      <p class="text-black font-bold">${i.time} Menit</p>
                              ...
                              (i, index) =>
                                `
                                    <button 
                                        data-id="${i.id}" 
                                        data-time="${i.time}" 
                                        data-price="${i.disc_price}" key=${index} 
                                        ...
                                        <div class="flex flex-col items-start">
                                                <p class="text-[#ED0226] font-bold">Rp${i.ori_price}</p>
                                                <p class="text-[#9CA9B9] line-through text-sm">Rp${i.disc_price}</p>
                                            </div>
                                        </div>
                                    </button>
                                `
                        ...
                        <button
                            id="myButton"
                            type="submit" 
                            class="w-full disabled:text-black bg-gray-600 text-white py-2 rounded-[100px] focus:outline-none h-[50px] cursor-pointer"
                        >
                            Beli
                        </button>
                    </div>
      ...
      didOpen: () => {
        ...
        if (pickedDenom === null) {
          button.disabled = true; // Disable the button if no denomination is selected
          button.classList.add("bg-gray-300", "opacity-45"); // Add styles for disabled button
          ...
          button.classList.remove("bg-gray-300", "opacity-45"); // Remove disabled styles
        ...
        buttons.forEach((button) => {
          button.addEventListener("click", () => {
            ...
            setPickedDenom(picked);
            Swal.getHtmlContainer()
              .querySelectorAll(".denom-btn")
              .forEach((btn) => {
                btn.classList.remove("border-[#EF2328]");
                btn.classList.remove("border-[3px]");
              });
            button.classList.add("border-[#EF2328]"); // Highlight the clicked button
            button.classList.add("border-[3px"); // Highlight the clicked button
          });
        });
      },
    });
